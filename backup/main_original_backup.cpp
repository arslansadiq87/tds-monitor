...
static const int PIN_LEVEL_ODD_ADC  = 32; // SW1/SW3/SW5/SW7/SW9 ladder
static const int PIN_LEVEL_EVEN_ADC = 33; // SW2/SW4/SW6/SW8 ladder
...
static const float LADDER_MATCH_TOLERANCE_V = 0.30f;
...
static float computeWaterLevelPercent(int oddSwitch, int evenSwitch) {
  if (oddSwitch <= 0 && evenSwitch <= 0) return NAN;

  float level = 0.0f;
  if (oddSwitch > 0 && evenSwitch > 0) {
    level = ((float)oddSwitch + (float)evenSwitch) * 0.5f;
  } else if (oddSwitch > 0) {
    level = (float)oddSwitch;
  } else {
    level = (float)evenSwitch;
  }

  return ((10.0f - level) / 9.0f) * 100.0f;
}
...
static void updateWaterLevelState(int oddSwitch, int evenSwitch) {
  ...
  latestWaterLevelPercent = computeWaterLevelPercent(oddSwitch, evenSwitch);
}
...
static void printWaterLevelDebug() {
  ...
  Serial.printf("GPIO32 odd ladder : %.3f V -> %s\n",
                latestOddLadderVoltage,
                latestOddSwitch > 0 ? ("SW" + String(latestOddSwitch)).c_str() : "open");
  Serial.printf("GPIO33 even ladder: %.3f V -> %s\n",
                latestEvenLadderVoltage,
                latestEvenSwitch > 0 ? ("SW" + String(latestEvenSwitch)).c_str() : "open");
  Serial.printf("Level decode      : %s\n", latestWaterLevelLabel.c_str());
  Serial.println();
}
...
void setup() {
  ...
  // Disabled during water-level ladder validation.
  ...
  Serial.println();
  Serial.println("[BOOT] Water-level test mode");
  Serial.println("GPIO32 = SW1/SW3/SW5/SW7/SW9");
  Serial.println("GPIO33 = SW2/SW4/SW6/SW8");
  Serial.println("Other runtime paths are commented out temporarily.");
  Serial.println();
}
